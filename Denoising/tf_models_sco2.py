...
def main(continue_training = False,denoising = True, conv_layer = True,ensemble = True, batch_size = 256,train_years = [2015,2016,2017,2018,2019], test_year = 2020,seed_offset = 5, save_path='/home/wkeely/OCO_L2DIA/'):
    ...
    for year in train_years:
        ...
        training_band_obs.append(file['sco2_band_obs'])
        training_band_no_eof.append(file['sco2_band_no_eof'])
        training_band_sim.append(file['sco2_band'])
    ...
    test_band_obs.append(file['sco2_band_obs'])
    test_band_no_eof.append(file['sco2_band_no_eof'])
    test_band_sim.append(file['sco2_band'])
    ...
    if ensemble:    
        ...
        for i in range(3):
            ...
            model_checkpoint_callback = tf.keras.callbacks.ModelCheckpoint(filepath=save_path + 'DAE'+model_str+'sco2_'+str(seed)+'.h5', save_weights_only=True, monitor='val_loss', mode='min', save_best_only=True)
        ...
        model_checkpoint_callback = tf.keras.callbacks.ModelCheckpoint(filepath=save_path + 'DAE'+model_str+'sco2.h5', save_weights_only=True, monitor='val_loss', mode='min', save_best_only=True)
        # model_checkpoint_callback = tf.keras.callbacks.ModelCheckpoint(filepath=save_path + 'DAE'+model_str+'sco2_'+str(seed)+'.h5', save_weights_only=True, monitor='val_loss', mode='min', save_best_only=True)
        model.fit(Xtrain,ytrain, epochs=40, batch_size=batch_size, shuffle=True, validation_data=(Xtest, ytest), callbacks=[RMSECallback(),  model_checkpoint_callback])
        test_pred = model.predict(Xtest)
        rmse_DAE = np.sqrt(np.mean((test_band_no_eof - test_pred)**2))
