...
from tensorflow.keras.layers import Conv1D, Conv1DTranspose, MaxPooling1D, UpSampling1D, concatenate, Flatten
...
class DenoisingConvAutoEncoder(Model):
    def __init__(self, shape):
        ...
        self.encoder.add(Conv1D(128, 3, activation='relu', padding='same', input_shape=(self.shape, 1)))
        self.encoder.add(MaxPooling1D(2, padding='same'))
        self.encoder.add(Conv1D(64, 3, activation='relu', padding='same'))
        self.encoder.add(MaxPooling1D(2, padding='same'))
        self.encoder.add(Conv1D(32, 3, activation='relu', padding='same'))
        self.encoder.add(Conv1D(32, 3, activation='relu', padding='same'))
        self.encoder.add(MaxPooling1D(2, padding='same'))
        ...
        self.decoder.add(Conv1D(32, 3, activation='relu', padding='same'))
        self.decoder.add(Conv1D(32, 3, activation='relu', padding='same'))
        self.decoder.add(UpSampling1D(2))
        self.decoder.add(Conv1D(64, 3, activation='relu', padding='same'))
        self.decoder.add(UpSampling1D(2))
        self.decoder.add(Conv1D(128, 3, activation='relu', padding='same'))
        self.decoder.add(UpSampling1D(2))
        self.decoder.add(Conv1D(1, 3, activation='linear', padding='same'))
...
def main(continue_training = False,denoising = True, conv_layer = True, ensemble=False,batch_size = 128,train_years = [2015,2016,2017,2018,2019], test_year = 2020, seed_offset = 5,save_path='/home/wkeely/OCO_L2DIA/'):
    ...
    for year in train_years:
        ...
        training_band_obs.append(file['wco2_band_obs'])
        training_band_no_eof.append(file['wco2_band_no_eof'])
        training_band_sim.append(file['wco2_band'])
    ...
    test_band_obs.append(file['wco2_band_obs'])
    test_band_no_eof.append(file['wco2_band_no_eof'])
    test_band_sim.append(file['wco2_band'])
    ...
    print('outcome_flag shape: ', outcome_flag.shape)
    ...
    # create the model
    shape = training_band_no_eof.shape[1]
    ...
    if conv_layer:
        ...
        model = DenoisingAutoEncoder(latent_dim=32, shape=shape, n_layers=8, nodes=[512,258,258,128,128,64,64])
    ...
    if ensemble:    
        ...
        for i in range(3):
            ...
            seed_wco2 = i+seed_offset
            opt = tf.keras.optimizers.Adam(learning_rate=0.00001)
            model = DenoisingUNET()
            model.compile(optimizer=opt, loss=losses.MeanSquaredError())
            model_checkpoint_callback = tf.keras.callbacks.ModelCheckpoint(filepath=save_path + 'DAE'+model_str+'wco2_'+str(seed_wco2)+'.h5', save_weights_only=True, monitor='val_loss', mode='min', save_best_only=True)
        ...
        model_checkpoint_callback = tf.keras.callbacks.ModelCheckpoint(filepath=save_path + 'DAE'+model_str+'wco2.h5', save_weights_only=True, monitor='val_loss', mode='min', save_best_only=True)
        # model_checkpoint_callback = tf.keras.callbacks.ModelCheckpoint(filepath=save_path + 'DAE'+model_str+'wco2_'+str(seed_wco2)+'.h5', save_weights_only=True, monitor='val_loss', mode='min', save_best_only=True)
        model.fit(Xtrain,ytrain, epochs=40, batch_size=batch_size, shuffle=True, validation_data=(Xtest, ytest), callbacks=[RMSECallback(),  model_checkpoint_callback])
        test_pred = model.predict(Xtest)
        rmse_DAE = np.sqrt(np.mean((test_band_no_eof - test_pred)**2))
