...
def main(args):
    ...
    dm = RetrievalDataModule(train_files, test_files, batch_size=args.batch_size, val_split=0.1, num_workers=args.num_workers)
    dm.setup('fit')
    ...
    if model_type == 'CondDiff':
        ...
        n_hidden_backbone = [300, 150, 50]
        n_hidden_guidance = [512, 512]

        beta_schedule = "linear"

        early_stopping_callback = EarlyStopping(monitor='val_loss', min_delta=0.00001, patience=50, mode='min', verbose=True)
        ...
        for m in range(M):
            ...
            cond_mean_model = DeterministicRegression(
                model=network, optimizer=partial(torch.optim.Adam, lr=lr), loss_fn=nn.MSELoss(), 
            )
            trainer = Trainer(
            ...
            card_model = CARDRegression(
                ...
                guidance_optim=partial(torch.optim.Adam, lr=lr),
            ...
            if M == 1:
                ...
                y_test_unnormed = y_test * y_std + y_mean

                rmse = np.sqrt(np.mean((pred_unnormed.detach().numpy() - y_test_unnormed.numpy()) ** 2))
                print(f'RMSE : {rmse}')

                results = {
                    'pred': pred_unnormed.detach().numpy(),
                    'uct': pred_uct,
                    'y_test': y_test_unnormed.numpy(),
                }
    ...
    if model_type == 'Evidential':
        ...
        early_stopping_callback = EarlyStopping(monitor='val_loss', min_delta=0.00001, patience=20, mode='min', verbose=True)
        # lr_monitor = LearningRateMonitor(logging_interval='step')
        checkpoint_callback = ModelCheckpoint(monitor="val_loss")

        network = MLP(n_inputs=x_dim, n_hidden=[256,128,128], n_outputs=4, activation_fn=nn.LeakyReLU())
    ...
    if model_type == 'SNGP':
        ...
        early_stopping_callback = EarlyStopping(monitor='val_loss', patience=20, mode='min', verbose=verbose)
        # lr_monitor = LearningRateMonitor(logging_interval='step')
    ...
    if model_type == 'DeepEnsemble':
        ...
        trained_models_nll = []
        for m in range(M):
            mlp_model = MLP(n_inputs = x_dim, n_hidden=[256, 128, 128], n_outputs=2, activation_fn=nn.LeakyReLU())
            ensemble_member = MVERegression(
                mlp_model, optimizer=partial(torch.optim.Adam, lr=1e-3), burnin_epochs=20
            )
            trainer = Trainer(
                max_epochs=150,
                limit_val_batches=0,
    ...
    if model_type == 'MLE':
        ...
        MLE = DeterministicRegression( mlp_model, optimizer=partial(torch.optim.Adam, lr=1e-3), loss_fn=nn.MSELoss())
        trainer = Trainer(
            max_epochs=epochs,
            enable_checkpointing=True,
            enable_progress_bar=True,
